...
function introduceAI() {
    ...
    setTimeout(() => {
        ...
        if (ball) {
            ...
            // Speak intro
            speak("Hello! I am FocusWin. I'm here to help you stay in the zone.");
        }
    }, 2000);
}

// Start intro on load
window.addEventListener('load', introduceAI);
...
if (SpeechRecognition) {
    ...
    recognition.onstart = function () {
        const ball = document.getElementById('magic-ball');
        if (ball) ball.classList.add('listening');
        speak("I'm listening...");
    };

    recognition.onend = function () {
        const ball = document.getElementById('magic-ball');
        if (ball) ball.classList.remove('listening');
    };
    ...
    recognition.onerror = function (event) {
        ...
        if (event.error === 'not-allowed') {
            alert('⚠️ Microphone access denied. Please check System Settings > Privacy > Microphone.');
    ...
    recognition.onresult = function (event) {
        ...
        fetch('/api/listen', {
            ...
            .then(data => {
                if (data.response) {
                    // Backend handles speaking, we just show text
                    const bubble = document.getElementById('speech-bubble');
                    if (bubble) {
    ...
    alert('⚠️ Voice control is not supported in this browser/mode.');
}
